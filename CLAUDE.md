...
- **`collect.py`** — Runs every 10s, polls Docker containers and scans `scientists/*/work/projects/` for pipeline progress. Detects busy status via MCP log recency (`/tmp/denario-mcp.log` mtime < 60s) and high CPU usage. Parses project titles from `paper.tex` (preferred) or `idea.md` (fallback). Plan-step status comes from the cmbagent_lg layout (`logs/step_N_verdict.json`, `step_N_researcher_verdict.json`, `deep_research_run.json` outcomes, `analysis_status.json` for halts, `codebase/step_N_failure_k.py` for attempts; wall time from verdict mtimes) with the legacy cmbagent `control/chats` path kept as fallback. Aggregates per-project costs from three sources: log files (`logs/{stage}_iter{N}.log`), CMBAgent JSON cost reports (`Iteration*/experiment_output/{planning,control}/cost/`), and EDA/compare_plans reports. Writes `status.json`.
